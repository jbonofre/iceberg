...
public class JdbcViewOperations extends BaseViewOperations {
  ...
  protected void doRefresh() {
    ...
    try {
      view = JdbcUtil.getTableOrView(false, connections, catalogName, viewIdentifier);
  ...
  protected void doCommit(ViewMetadata base, ViewMetadata metadata) {
    ...
    try {
      Map<String, String> view =
          JdbcUtil.getTableOrView(false, connections, catalogName, viewIdentifier);
  ...
  private void createView(String newMetadataLocation) throws SQLException, InterruptedException {
    ...
    if (JdbcUtil.tableExists(catalogName, connections, viewIdentifier)) {
      throw new AlreadyExistsException("Table with same name already exists: %s", viewIdentifier);
    }

    int insertRecord =
        JdbcUtil.doCommitCreateTableOrView(
            false, connections, catalogName, namespace, viewIdentifier, newMetadataLocation);
